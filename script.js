...
function updateCartUI() {
  ...
  cart.forEach(item => {
    ...
    `;
    div.querySelectorAll('.cart-item-controls button')[0].onclick = () => updateCartQuantity(item.id, item.qty - 1);
    div.querySelectorAll('.cart-item-controls button')[1].onclick = () => updateCartQuantity(item.id, item.qty + 1);
    div.querySelectorAll('.cart-item-controls button')[3].onclick = () => removeFromCart(item.id);
...
async function loadProducts() {
  try {
    ...
    products.forEach(p => productsMap.set(p.id, p));  // ← NEW LINE
    return products;
  } catch (err) {
...
updateCartUI();
  ...
  if (orderRow && window.currentProduct) {
    const isUpcoming = window.currentProduct.availability === 'Upcoming';
    const isOOS = Number(window.currentProduct.stock) <= 0 && window.currentProduct.availability !== 'Pre Order';
    if (!isUpcoming) {
      const addBtn = document.createElement('button');
      addBtn.className = 'add-to-cart-btn';
